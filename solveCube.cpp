...
extern "C" int solveCube(char state[20][4],char result[1000]);
//----------------------------------------------------------------------
        ...
        if( applicableMoves[phase] & (1 << move) ){
          ...
          if( newDir  &&  newDir != oldDir ){
            ...
            for( int i=0; i<(int)algorithm.size(); i++ ){
              result[resultPointer++]="UDFBLR"[algorithm[i]/3];
              result[resultPointer++]=char(algorithm[i]%3+1+int('0'));
